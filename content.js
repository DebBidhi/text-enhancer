...
function init() {
  ...
  chrome.storage.sync.get(['apiKey', 'selectedModel', 'customModel'], (result) => {
...
function createEnhancerPopup() {
  ...
  enhancerPopup.querySelector('.text-enhancer-close').addEventListener('click', hidePopup);
...
function isEditableElement(element) {
  ...
  return (
    tagName === 'input' && ['text', 'search', 'url', 'tel', 'email', 'password', ''].includes(element.type) ||
    tagName === 'textarea' ||
...
function showPopup() {
  ...
  if (isVisible) {
    ...
    enhancerPopup.style.top = `${(window.innerHeight - popupRect.height) / 2}px`;
...
async function enhanceText() {
  ...
  if ((selectedModel === 'openai' || selectedModel === 'anthropic') && !apiKey) {
  ...
  try {
    ...
    if (selectedModel === 'openai') {
      ...
      if (!customModel) {
        throw new Error('Custom model name not set. Please set it in the extension popup.');
      }
      enhancedText = await callOllamaCustom(inputText, instruction, customModel);
    } else {
...
function replaceText(newText) {
  ...
  if (isTwitter) {
    ...
    if (start !== end) {
      activeElement.value = oldValue.substring(0, start) + newText + oldValue.substring(end);
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful assistant that enhances text. Follow the instruction precisely and modify the text accordingly. Provide only the modified text as your response, without any additional explanations or formatting.'
        },
        {
          role: 'user',
...
async function callOllamaCustom(text, instruction, customModel) {
  try {
    ...
    if (response && response.success) {
      ...
      throw new Error(response?.error || 'Unknown error in Ollama API response');
    }
  } catch (error) {
