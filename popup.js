document.addEventListener("DOMContentLoaded", function () {
  ...
  chrome.storage.sync.get(
    ...
    function (result) {
      if (result.selectedModel) {
        ...
        const customModelDiv = document.getElementById(
          "custom-model-container"
        );
        const apiConDiv = document.getElementById("text_enh_api");
        customModelDiv.style.display =
          result.selectedModel === "ollama model:custom" ? "block" : "none";
        apiConDiv.style.display = ["openai", "anthropic"].includes(
  ...
  modelSelect.addEventListener("change", function () {
    ...
    const apiConDiv = document.getElementById("text_enh_api");
    customModelDiv.style.display =
      selectedModel === "ollama model:custom" ? "block" : "none";
    apiConDiv.style.display = ["openai", "anthropic"].includes(selectedModel)
      ? "block"
      : "none";
  });
  ...
  function toggleApiKeyInput(model) {
    ...
    apiKeyInput.placeholder =
      isOllama || isCustomModel
        ? "No API key needed for Ollama"
        : "Enter your API key";

    // Show or hide the custom model name input field
    const customModelDiv = document.querySelector(
